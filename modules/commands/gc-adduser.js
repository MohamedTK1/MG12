module.exports.config = {
	...
	hasPermssion: 1,
  credits: "S H A D O W",
	description: "اضافه شخص للمجموعه",
...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("اكتب معرف او رابط الشخص الذي تريد اضافته");
	if (!isNaN(args[0])) return adduser(args[0], undefined);
	else {
		try {
			var [id, name, fail] = await getUID(args[0], api);
			if (fail == true && id != null) return out(id);
			else if (fail == true && id == null) return out("المعرف غير موجود")
			else {
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`${name ? name : "العضو"} موجود بالفعل`);
		else {
			...
			try {
				...
				return out(`لا يمكن اضافه ${name ? name : "العضو"} للمجموعه`);
			}
			if (approvalMode === true && !admins.includes(botID)) return out(`اضافه ${name ? name : "العضو"} للمستخدمين !`);
			else return out(`تمت اضافه ${name ? name : "العضو"} للمجموعه !`)
		}
	}
    }