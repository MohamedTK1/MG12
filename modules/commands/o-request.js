...
api.changeNickname(`${(!global.config.BOTNAME) ? "Made by shadow" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
...
module.exports.run = async function({ api, event, args, permission, handleReply }) {
    ...
    case "t":
    case "المجموعات": {
   const permission = ["100030029174216","100030029174216"];
    if (!permission.includes(event.senderID)) return api.sendMessage("ماعدك صلاحية أستعمال هذا الامر  :>", event.threadID, event.messageID);
     const { threadID, messageID } = event;
    const commandName = this.config.name;
    var msg = "", index = 1;
